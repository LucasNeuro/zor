...
export default function ConteudoPage() {
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden" style={{ background: "#f8fcf6" }}>
      {/* Body */}
      <div className="flex-1 overflow-y-auto p-6">
        <div className="grid grid-cols-3 gap-4 mb-8">
          {CATEGORIAS.map(cat => (
            <div key={cat.label} className="rounded-xl p-5 cursor-default transition-all hover:scale-[1.02]"
              style={{ background: "#ffffff", border: `1px solid ${cat.cor}30` }}>
              <div className="text-3xl mb-3">{cat.icon}</div>
              <h3 className="font-black text-sm mb-1" style={{ color: "#0b2210" }}>{cat.label}</h3>
              <p className="text-xs leading-relaxed" style={{ color: "#5d7a67" }}>{cat.descricao}</p>
              <div className="mt-3 text-xs font-bold" style={{ color: cat.cor + "80" }}>Em desenvolvimento</div>
            </div>
          ))}
        </div>

        <div className="rounded-xl p-6 text-center" style={{ background: "#ffffff", border: "1px solid #dcebd8" }}>
          <p className="text-4xl mb-3">✏️</p>
          <p className="font-black text-base mb-2" style={{ color: "#0b2210" }}>Módulo de Conteúdo</p>
          <p className="text-sm max-w-md mx-auto" style={{ color: "#5d7a67" }}>
