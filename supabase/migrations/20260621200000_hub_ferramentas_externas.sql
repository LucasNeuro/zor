-- Integrações externas e ferramentas HTTP por tenant (MVP: webhook_generico + hub_ext_*).
-- Acesso via service role nas APIs Hub (sem RLS), como hub_ferramentas_custom.
...
COMMENT ON TABLE public.hub_integracoes IS
  'Instâncias de integração por tenant. integracao_id = tipo (webhook_generico, gmail, …). Credenciais em hub_integracao_credenciais.';
...
COMMENT ON TABLE public.hub_integracao_credenciais IS
  'Credenciais por instância de integração. MVP: JSONB em texto; em produção encriptar (Vault/KMS).';
COMMENT ON COLUMN public.hub_integracao_credenciais.credenciais IS
  'MVP plaintext JSONB, ex.: {"api_key":"…","api_key_header":"X-API-Key"} ou {"bearer_token":"…"}. Produção: encriptar.';
...
COMMENT ON TABLE public.hub_ferramentas_externas IS
  'Ferramentas Mistral que executam HTTP com credenciais da integração ligada (hub_ext_*).';
