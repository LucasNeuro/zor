...
const HEADER_SURFACE =
  "sticky top-0 z-20 flex-shrink-0 border-b border-[#d7e3da]/90 bg-[#f4f9f2]/95 backdrop-blur-md supports-[backdrop-filter]:bg-[#f4f9f2]/88 shadow-[inset_0_-1px_0_0_rgba(201,162,74,0.18)]";
...
/**
 * CRM page title strip: superfície clara TIVIA com detalhe dourado, fixo ao topo da área rolável.
 */
    ...
    <header className={`${HEADER_SURFACE} px-3 py-3 sm:px-5 sm:py-3.5 ${className}`.trim()}>
      <div className="flex flex-col gap-3 min-[480px]:flex-row min-[480px]:items-start min-[480px]:justify-between min-[480px]:gap-4">
        <div className="min-w-0 flex-1">
          <div className="text-base sm:text-lg font-black tracking-tight text-[#12382b]">{title}</div>
          {description != null && <div className="mt-0.5 text-xs text-[#5f7469]">{description}</div>}
        </div>
        {actions != null && (
