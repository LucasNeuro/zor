-- Copiloto IA (briefing): colar no Supabase → SQL Editor → Run
-- Ficheiro espelho: supabase/migrations/20260621170000_ensure_hub_briefing_chat_tables.sql
...
CREATE INDEX IF NOT EXISTS idx_hub_briefing_sessao_agente
  ON public.hub_crm_agente_briefing_sessao(agente_slug, atualizado_em DESC);

CREATE INDEX IF NOT EXISTS idx_hub_briefing_msg_sessao
  ON public.hub_crm_agente_briefing_mensagem(sessao_id, criado_em);
